...
export class Dashboard1Component {
    ...
    getTotalRecords() {
        setTimeout(() => {
            this.main.getAllReports().subscribe(res => {
                if (res) {
                    this.totalCases = res['reports'][0].cases
                    this.toatalDeath = res['reports'][0].deaths
                    this.totalRecovered = res['reports'][0].recovered
                    this.active_cases = res['reports'][0].active_cases[0].currently_infected_patients
                    this.doughnutChartData.push(this.totalCases, this.toatalDeath, this.totalRecovered, this.active_cases)
                }
                this.countyTabel = res['reports'][0].table[0]
                this.findRecord = _.find(this.countyTabel, { Country: 'World' })
                this.rows = this.countyTabel
                this.countyTabel.map(x => {
                    this.counties.push(x.Country)
                })
    ...
    getCountyData(countrie) {
        var data = []
        this.findRecord = []
        this.doughnutChartData = []
        this.findRecord = _.find(this.countyTabel, { Country: countrie })
        data.push(this.findRecord.TotalCases, this.findRecord.TotalDeaths, this.findRecord.TotalRecovered, this.findRecord.ActiveCases)
        for (let i = 0; i < data.length; i++) {
            const element = data[i];
            this.doughnutChartData.push(element.replace(/[^\d\.\-]/g, ""))
        }
    }
    onSelect(e) {
